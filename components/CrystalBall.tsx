...
export default function CrystalBall({
  ...
  const monthlyProfit =
    dealsPerMonth * avgAssignmentFee * (netProfitMargin / 100) * ownershipFactor;
  ...
  return (
    <div className="bg-white p-6 rounded-2xl shadow space-y-6 flex flex-col items-center">
      ...
      <div
        onClick={handleClick}
        className={`relative w-40 h-40 cursor-pointer rounded-full flex items-center justify-center transition-transform duration-500 ${
          isAnimating ? "animate-zoom-crystal" : "hover:scale-105"
        }`}
        title="Click the crystal ball to reveal forecast"
      >
        {/* Glowing aura */}
        <div
          className={`absolute w-48 h-48 rounded-full bg-gradient-to-r from-purple-400 via-blue-400 to-indigo-400 opacity-50 blur-3xl transition-all ${
            isAnimating ? "animate-pulse" : "hover:opacity-70"
          }`}
        ></div>

        {/* Inner reflection/light streak */}
        <div className="absolute w-40 h-40 rounded-full bg-white/20 rounded-t-full transform rotate-12 pointer-events-none"></div>

        {/* Reveal Forecast text */}
        {!showResults && (
          <span
            className={`absolute text-center font-extrabold text-xl px-2 text-cyan-50 drop-shadow-[0_0_8px_rgba(0,0,0,0.7)] animate-glow ${
      ...
      <style jsx>{`
        ...
        .animate-zoom-crystal {
          animation: zoomCrystal 2.5s ease-in-out forwards;
        }
        ...
        @keyframes glow {
          0%,
          100% {
            text-shadow: 0 0 6px #e0f7ff, 0 0 12px #a0eaff, 0 0 24px #50d8ff;
          }
          50% {
            text-shadow: 0 0 12px #e0f7ff, 0 0 24px #a0eaff, 0 0 36px #50d8ff;
          }
        }
